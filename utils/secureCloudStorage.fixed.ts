    ...
    if (await this.isSyncEnabled() && auth.currentUser) {
      ...
      if (networkState.isConnected) {
        try {
          ...
          // Si la récupération cloud réussit, synchroniser avec le stockage local
          // pour assurer la cohérence des données
          await this.mergeMeasurements(cloudMeasurements);
          
          return cloudMeasurements;
        } catch (error) {
          console.warn('Échec de récupération cloud, utilisation du stockage local:', error);
        }
      }
    }
    
    // Récupérer du stockage local par défaut ou si le cloud échoue
    return await getStoredMeasurementsLocal();
  }

  // Supprimer une mesure localement et dans le cloud
  static async deleteMeasurement(id: string): Promise<string> {
    // Toujours supprimer localement
    await removeMeasurementLocal(id);
    
    // Si en ligne et synchronisation activée, essayer de supprimer du cloud
    if (await this.isSyncEnabled() && auth.currentUser) {
      const networkState = await NetInfo.fetch();
      ...
      if (networkState.isConnected) {
        try {
          ...
          // Si la suppression cloud échoue, ajouter aux opérations en attente
          await this.addPendingOperation({
            type: 'delete',
            measurementId: id,
            timestamp: Date.now()
